...
def get_db_connection():
    """Connect to PostgreSQL via Streamlit secrets."""
    return psycopg2.connect(st.secrets["POSTGRES_URL"])

# ——— INITIALIZE DATABASE (users + logs) ———
def init_db():
...
# ——— LOGGED IN: SIDEBAR NAVIGATION (CLEAN) ———
...
with st.form("log_form"):
    ...
    if st.form_submit_button("Log Session"):
        ...
        try:
            ...
            """, (
                st.session_state.user_id, date, distance,
                run_min, run_sec, pushups, crunches, felt
            ))
            conn.commit()
