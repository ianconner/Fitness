...
def sidebar():
    ...
    if st.sidebar.button("Home", key="home", use_container_width=True):
        st.rerun()
    if st.sidebar.button("Dashboard", key="dash", use_container_width=True):
        st.switch_page("pages/01_Dashboard.py")
    if st.sidebar.button("Log Workout", key="log", use_container_width=True):
        st.switch_page("pages/02_Log_Workout.py")
    if st.sidebar.button("Goals", key="goals", use_container_width=True):
        st.switch_page("pages/03_Goals.py")
    if st.sidebar.button("SOPHIA Coach", key="coach", use_container_width=True):
        st.switch_page("pages/04_AI_Coach.py")
    if st.session_state.role == 'admin':
        if st.sidebar.button("Admin", key="admin", use_container_width=True):
...
if not st.session_state.logged_in:
    ...
    st.info("Use the sidebar to navigate.")# .strealit/app.py
import streamlit as st
import psycopg2
