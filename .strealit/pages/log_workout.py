...
def main():
    ...
    with st.form("log_workout_form", clear_on_submit=True):
        ...
        overall_notes = st.text_area("Notes (for AI Coach context)")
        duration_min = st.number_input("Duration (min)", min_value=1, value=30)

        # Initialize exercises
        exercises = st.session_state.get("exercises", [])
        if not exercises:
            exercises = [{"category": None, "sub_category": None, "exercise": "", "sets": 3, "reps": 10, "weight_lbs": 0.0, "time_min": 0.0, "rest_min": 1.5, "distance_mi": 0.0, "pace_min_mi": 0.0}]

        for i, ex in enumerate(exercises):
            st.markdown(f"**Exercise {i+1}**")
            
            # Category Selection (store in ex for reactivity)
        ...
        with col_add:
            if st.form_submit_button("Add Exercise"):
                exercises.append({"category": None, "sub_category": None, "exercise": "", "sets": 3, "reps": 10, "weight_lbs": 0.0, "time_min": 0.0, "rest_min": 1.5, "distance_mi": 0.0, "pace_min_mi": 0.0})
