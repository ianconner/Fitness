...
def main():
    ...
    if "exercises" not in st.session_state:
        st.session_state.exercises = [{"category": None, "sub_category": None, "exercise": "", "sets": 1, "reps": 1, "weight_lbs": 0.0, "time_min": 0.0, "rest_min": 1.5, "distance_mi": 0.0, "pace_min_mi": 0.0, "notes": ""}]
    ...
    for i, ex in enumerate(exercises):
        with st.container(border=True):
            ...
            if ex["category"] == "Cardio":
                ...
                ex["distance_mi"] = 0.0
                ex["pace_min_mi"] = 0.0
                ex["rest_min"] = 1.5  # Default rest for weights
            ...
            ex["notes"] = st.text_area("Notes (AI Context - Amplification)", value=ex["notes"], key=notes_key, help="Amplifying details for RISE Coach (e.g., 'Trail was muddy, focused on form; RPE 7/10')")
    ...
    with col_add:
        if st.button("Add Exercise"):
            exercises.append({"category": None, "sub_category": None, "exercise": "", "sets": 1, "reps": 1, "weight_lbs": 0.0, "time_min": 0.0, "rest_min": 1.5, "distance_mi": 0.0, "pace_min_mi": 0.0, "notes": ""})
        ...
        if submitted:
            if not any(ex["exercise"].strip() and ex["category"] for ex in exercises):
                ...
                try:
                    ...
                    # Insert current exercises (partial save)
                    for ex in exercises:
